...
const getBranch = async (req, res) => {
  try {
    let branches = await Branch.find({ flag: true });
...
const getBranchID = async (req, res) => {
  try {
    let branches = await Branch.find({ name: req.body.name });
...
const addBranch = async (req, res) => {
  let { name, branchCode } = req.body;
  try {
    let branch = await Branch.findOne({ branchCode });
    if (branch) {
      const data = {
        success: false,
...
module.exports = { getBranch, addBranch, deleteBranch, deactivateBranch, getBranchID };
